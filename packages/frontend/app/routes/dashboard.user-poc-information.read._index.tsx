...
import type { Route } from "./+types/dashboard.user-poc-information.read._index";
...
export const clientLoader = async ({ request }: Route.ClientLoaderArgs) => {
  ...
  const res = await client.api.v1["user-poc-information"].$get({
    query: { limit, offset },
  });
  if (res.ok) {
...
//   { title: "User POC Information Read" },
//   { content: "User POC Information Read | description", name: "description" },
// ];
            ...
            <tbody>
              {loaderData.data.data.map((value) => (
                <tr key={value.id}>
                  ...
                  <td>{value.attributes?.userId}</td>
                  <td>
                    <div className="join">
                      <Link
                        className="btn btn-ghost btn-xs join-item"
                        to={href("/dashboard/user-poc-information/:id/delete", {
                      ...
                      <Link
                        className="btn btn-ghost btn-xs join-item"
                        to={href("/dashboard/user-poc-information/:id/read", {
                          id: value.id.toString(),
                        })}
                      >
                        DETAILS
                      ...
                      <Link
                        className="btn btn-ghost btn-xs join-item"
                        to={href("/dashboard/user-poc-information/:id/update", {
                          id: value.id.toString(),
                        })}
                      >
                        UPDATE
