...
import type { Route } from "./+types/dashboard.user-poc-information.$id.delete";

// export const action = async ({ params }: Route.ActionArgs) => {
//   console.log("SERVER - action");
//   const client = hc<AppType>("http://127.0.0.1:8081/");
//   const res = await client.api.v1["user-poc-information"][":id"].$delete({ param: params });
//   if (res.ok) {
...
export const clientAction = async ({ params }: Route.ClientActionArgs) => {
  ...
  await sleep(1000);
  const client = hc<AppType>("http://127.0.0.1:8081/");
  const res = await client.api.v1["user-poc-information"][":id"].$delete({
    param: params,
  });
  if (res.ok) {
    const { data } = await res.json();
...
//   { title: `User POC Information Delete #${params.id}` },
//   {
//     content: `User POC Information Delete #${params.id} | description`,
//     name: "description",
//   },
// ];
      ...
      <fetcher.Form method="post">
        <fieldset className="bg-base-200 border border-base-300 fieldset p-4 rounded-box">
          <legend className="fieldset-legend">
            User POC Information #{params.id} Delete
          </legend>
